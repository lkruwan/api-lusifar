...
router.get('/nhentaipdf', async (req, res) => {
	var code = req.query.code
	var nhres = `https://lusifar-api.herokuapp.com/nsfw/nhcode?query=${code}`
	var nhread = `https://lusifar-api.herokuapp.com/nsfw/nhread?query=${code}`
	res.json({
		pdf: nhres,
		read: nhread,
...
router.get('/nhcode', async (req, res) => {
        try {
	var query = req.query.query
	let data = await axios.get('https://lusifar-api.herokuapp.com/nsfw/nHentai?code='+query)
